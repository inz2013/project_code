...
static void counter_to_struct( uint32_t sec, RTC_t *t )
{
	...
	t->month = (current_year % 365)/30;				// 1..12
	t->mday = day + 1;				// 1..31
}
...
static uint32_t struct_to_counter( const RTC_t *t )
{
	...
	//ustawienie czasu startowego na ten ustawiny przez set_time()
	result -= current_year;
...
bool rtc_gettime (RTC_t *rtc)
{
	...
	while ( ( t = xTaskGetTickCount() ) != xTaskGetTickCount() ) { ; }
	counter_to_struct( t/1000, rtc ); // get non DST time
	adjustDST( rtc );
...
{
	cnt = cnt*1000;
	xTaskSetTickCount( (portTickType) cnt, 0 );
}
...
bool rtc_settime (const RTC_t *rtc)
{
	...
	FIRSTYEAR = rtc->year;
	current_year = get_current_year(rtc);
	xTaskSetTickCount( (portTickType) cnt, 1 );
